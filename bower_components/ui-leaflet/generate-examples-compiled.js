...
var isAnExample = function isAnExample(filename) {
...
var isJavascript = function isJavascript(filename) {
    return (/.*\.js/.test(filename)
    );
};

var deleteFileIfJavascript = function deleteFileIfJavascript(filename) {
    var df = Q.defer();
...
var cleanJavascriptFilesFromControllersDirectory = function cleanJavascriptFilesFromControllersDirectory(dir) {
    var df = Q.defer();
...
var writeController = function writeController(script, examplefile, controllers_directory) {
    var df = Q.defer();
...
var generateControllersFromExamples = function generateControllersFromExamples(examples_directory, controllers_directory) {
    var df = Q.defer();
...
var extractId = function extractId(filename) {
    var arr = filename.replace('.html', '').split('-');
    arr.splice(0, 2);
    return arr.join('-');
};

var extractTitle = function extractTitle(filename) {
    var html = fs.readFileSync(path.join(__dirname, 'examples', filename));
    var title;
...
var extractDescription = function extractDescription(filename) {};

var extractDate = function extractDate(filename) {
    var stats = fs.statSync(filename);
    return stats.mtime;
};

var generateExamplesJSONFile = function generateExamplesJSONFile(examples_directory, json_file) {
    var df = Q.defer();
    var examples = {};
    fs.readdir(examples_directory, function (err, list) {
        list.forEach(function (filename) {
...
//# sourceMappingURL=generate-examples-compiled.js.map