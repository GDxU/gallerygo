...
MdIconService['$inject'] = ["config", "$templateRequest", "$q", "$log", "$mdUtil", "$sce"];angular.module('material.components.icon').constant('$$mdSvgRegistry', {
...
MdIconProvider.prototype = {
  icon: function icon(id, url, viewBoxSize) {
  ...
  iconSet: function iconSet(id, url, viewBoxSize) {
    config[id] = new ConfigurationItem(url, viewBoxSize);
    return this;
  },

  defaultIconSet: function defaultIconSet(url, viewBoxSize) {
    var setName = '$default';
  ...
  defaultViewBoxSize: function defaultViewBoxSize(viewBoxSize) {
  ...
  function loadByURL(url) {
    ...
    function loadByHttpUrl(url) {
      return $q(function (resolve, reject) {
        ...
        var announceAndReject = function announceAndReject(err) {
          var msg = angular.isString(err) ? err : err.message || err.data || err.statusText;
          $log.warn(msg);
          reject(err);
        },
            extractSvg = function extractSvg(response) {
          if (!svgCache[url]) {
...
//# sourceMappingURL=icon-compiled.js.map