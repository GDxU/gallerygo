...
describe('ng-pageslide: ', function () {
  ...
  beforeEach(function (done) {
    module('pageslide-directive', ['$provide', function ($provide) {
      $provide.value('$document', $document);
    }]);
    compilePageslide = function compilePageslide(html) {
      inject(['$compile', '$rootScope', '$document', '$timeout', function (_$compile_, $rootScope, $document, _$timeout_) {
        $compile = _$compile_;
        $timeout = _$timeout_;
        scope = $rootScope.$new();
    ...
    describe('when the element is invalid', function () {
      describe('because there is no content inside of the root element', function () {
        it('should throw an exception for no content', function (done) {
          expect(function () {
            compilePageslide('<pageslide></pageslide>');
          }).toThrow();
          done();
        });
      });
      describe('because the root element is not a div', function () {
        it('should throw an exception for no content', function (done) {
          expect(function () {
            compilePageslide('<p pageslide></p>');
          }).toThrow();
          done();
        });
      });
    ...
    describe('when the element is valid', function () {
      describe('and has defined the container', function () {
        beforeEach(function (done) {
          ...
          compilePageslide(['<div pageslide ps-container="customContainer" ps-open="is_open">', '<div>test</div>', '</div>'].join(''));
          done();
        });
        ...
        it('should contain the pageslide with the custom defined container', function (done) {
          expect(angular.element(document.querySelector('#customContainer')).html()).toContain('pageslide');
          done();
        });
      ...
      describe('and has defined the body class', function () {
        ...
        beforeEach(function (done) {
          compilePageslide(['<div pageslide ps-body-class="customBodyClass" ps-class="customBodyClass" ps-open="is_open">', '<div>test</div>', '</div>'].join(''));
          done();
        });
      ...
      describe('and has set push to true', function () {
        beforeEach(function (done) {
          compilePageslide(['<div pageslide ps-push="true" ps-open="is_open">', '<div>test</div>', '</div>'].join(''));
          done();
        });
      ...
      describe('when psAutoClose is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<pageslide ps-auto-close="true" ps-open="is_open">', '<div>test</div>', '</pageslide>'].join(''));
          done();
        });
      ...
      describe('when onopen is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<pageslide onopen="callback" ps-open="is_open">', '<div>test</div>', '</pageslide>'].join(''));
          done();
        });
      ...
      describe('when onclose is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<pageslide onclose="callback" ps-open="is_open">', '<div>test</div>', '</pageslide>'].join(''));
          done();
        });
        it('should call onOpen callback after transition', function (done) {
          scope.is_open = false;
  ...
  describe('functionality', function () {
    describe('default/right pageslide', function () {
      it('Should attach the pageslide panel to <body>', function (done) {
        ...
        compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-speed="-1.5">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
      ...
      it('Should open and close watching for ps-open', function (done) {
        // Create template DOM for directive
        compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-speed="0.5" href="#target">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
      ...
      it('Should attach the pageslide inner content to <body>', function (done) {
        // Create template DOM for directive
        compilePageslide(['<pageslide ps-open="is_open">', '<div>', '<p>some random content...</p>', '</div>', '</pageslide>'].join(''));
      ...
      describe('when binding the key listener', function () {
        beforeEach(function (done) {
          // Create template DOM for directive
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-key-listener="true">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
    ...
    describe('left pageslide', function () {
      describe('by default', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="left">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
      ...
      describe('when push is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="left" ps-push="true">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
    ...
    describe('top pageslide', function () {
      describe('by default', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="top">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
      ...
      describe('when push is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="top" ps-push="true">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
    ...
    describe('bottom pageslide', function () {
      describe('by default', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="bottom">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
      ...
      describe('when push is set', function () {
        beforeEach(function (done) {
          compilePageslide(['<div>', '<div pageslide ps-open="is_open" ps-side="bottom" ps-push="true">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
          done();
        });
    ...
    xit('Should sync ps-open state between pageslide\'s scope and parent scope', function () {
      ...
      // Create template DOM for directive
      compilePageslide(['<div>', '<div pageslide="right" ps-open="is_open" ps-speed="0.5" href="#target">', '<div id="target">', '<p>some random content...</p>', '<a id="target-close" href="#">Click to close</a>', '</div>', '</div>', '</div>'].join(''));
...
//# sourceMappingURL=angular-pageslide-directive.spec-compiled.js.map